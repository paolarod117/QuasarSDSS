    ...
    for jjjs in jjj:
        ...
        if(
            ...
            if part >= countBI and bb == 0:#code will go through this only when hte absorption is wider than 600km/s.
                ...
                if beta[jjjs] < maxvel and beta[jjjs] > minvel: #Recheck on BAL v range
                ...
                ####################################################################################################code counts from the left
                    ...
                    obs_wavelength_C_final=beta[obs_wavelength_C_index]+countBI
                    plot((obs_wavelength_C_final, obs_wavelength_C_final),(-1,2),'k-')#I have added this line (11/08/2016)

                    vvmins=np.min(where(beta > beta[jjjs]+countBI))#the logic of this line is: the value of speed the code is at right now is obviously 600km/s to the right of the vmin of the absorption feature (this is the index value)
